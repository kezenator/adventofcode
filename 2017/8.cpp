...
const std::string EXAMPLE = R"(b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10)";

const std::string PUZZLE_INPUT = R"(o inc 394 if tcy >= -3
s inc -204 if s <= -5
keq inc -34 if keq > -6
ip inc 762 if ip == 4
zr inc -644 if ip >= 6
...
struct Instruction
{
    explicit Instruction(const std::string &str)
    {
        ...
        reg = words[0];
        op = words[1];
        amount = std::atoi(words[2].c_str());
        test_reg = words[4];
        test_op = words[5];
        test_amount = std::atoi(words[6].c_str());
    }
    ...
    void execute(CpuState  &state)
    {
        ...
        if (test_op == "<")
            test_result = state.reg(test_reg) < test_amount;
        else if (test_op == "<=")
            test_result = state.reg(test_reg) <= test_amount;
        else if (test_op == ">")
            test_result = state.reg(test_reg) > test_amount;
...
int highest_in_state(const CpuState &state)
{
    int result = state.registers.begin()->second;
...
int largest_after_run(const std::string &instructions)
{
    std::vector<std::string> lines;
    boost::split(lines, instructions, boost::is_any_of("\n"));

    CpuState state;

    for (const auto &line : lines)
    {
        Instruction(line).execute(state);
    }

    return highest_in_state(state);
}

int highest_during_run(const std::string &instructions)
{
    std::vector<std::string> lines;
    boost::split(lines, instructions, boost::is_any_of("\n"));
    ...
    {
        Instruction(line).execute(state);
        int highest = highest_in_state(state);
...
{
    assert(largest_after_run(EXAMPLE) == 1);
    assert(highest_during_run(EXAMPLE) == 10);

    std::cout << "Answer #1: " << largest_after_run(PUZZLE_INPUT) << std::endl;
    std::cout << "Answer #2: " << highest_during_run(PUZZLE_INPUT) << std::endl;
