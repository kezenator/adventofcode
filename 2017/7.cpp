...
const std::string EXAMPLE = R"(pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
const std::string PUZZLE_INPUT = R"(jovejmr (40)
fesmk (24)
gwhfv (74)
...
{
public:
    explicit Programme(const std::string &name)
        : m_name(name)
        , parent()
    {
    }

    std::shared_ptr<Programme> parent;
    std::vector<std::shared_ptr<Programme>> children;
    int weight;

    const std::string &Name() const { return m_name; }

    int TowerWeight() const
    {
        int result = weight;
        for (const auto &child : children)
            result += child->TowerWeight();
        return result;
    }
...
{
public:
    explicit Tower(const std::string &description)
    {
        ...
        for (const std::string &line : lines)
        {
            ...
            std::shared_ptr<Programme> prog_ptr = GetProgramme(words[0]);

            prog_ptr->weight = std::atoi(words[1].substr(1, words[1].size() - 2).c_str());

            for (int index = 3; index < words.size(); ++index)
            {
                std::string child_name = words[index];
                if (child_name.back() == ',')
                    child_name.pop_back();

                std::shared_ptr<Programme> child_ptr = GetProgramme(child_name);

                assert(!child_ptr->parent);

                child_ptr->parent = prog_ptr;
                prog_ptr->children.push_back(child_ptr);
            }
        }
    }

    std::string NameOfBottom()
    {
        ...
        assert("Invalid tower" && false);
    }

    int FixedWeight()
    {
        int smallest_weight_fix = std::numeric_limits<int>::max();
        int result = 0;

        for (const auto &entry : m_map)
        {
            std::shared_ptr<Programme> prog_ptr = entry.second;

            if (prog_ptr->children.size() > 2)
            {
                std::vector<int> weights;
                for (const auto &child : prog_ptr->children)
                    weights.push_back(child->TowerWeight());

                for (size_t index = 0; index < weights.size(); ++index)
                {
                    if ((weights[index] != weights[(index + 1) % weights.size()])
                        && (weights[index] != weights[(index + 2) % weights.size()]))
                    {
                        // This weight is the odd one out - return it's weight corrected
                        // by the difference

                        int current = prog_ptr->children[index]->weight;
                        int correct = weights[(index + 1) % weights.size()];
                        int wrong = weights[index];

                        if (correct < smallest_weight_fix)
                        {
                            smallest_weight_fix = correct;
                            result = current + correct - wrong;
                        }
                    }
                }
            }
...
private:
    ...
    std::shared_ptr<Programme> GetProgramme(const std::string &name)
    {
        auto it = m_map.find(name);
        if (it == m_map.end())
        {
            m_map[name] = std::make_shared<Programme>(name);
            return m_map[name];
        }
        return it->second;
    }

    std::map<std::string, std::shared_ptr<Programme>> m_map;
};

int main(int /*argc*/, const char */*argv*/[])
{
    assert(Tower(EXAMPLE).NameOfBottom() == "tknk");
    assert(Tower(EXAMPLE).FixedWeight() == 60);

    std::cout << "Answer #1: " << Tower(PUZZLE_INPUT).NameOfBottom() << std::endl;
    std::cout << "Answer #2: " << Tower(PUZZLE_INPUT).FixedWeight() << std::endl;
