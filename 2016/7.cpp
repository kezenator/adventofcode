...
const std::string input = R"(dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
bool supports_tls(const std::string &str)
{
    ...
    for (size_t index = 0; index <= (str.size() - 4); ++index)
    {
    ...
    return got_outside && !got_inside;
}

bool supports_ssl(const std::string &str)
{
    size_t bracket_count = 0;

    std::set<std::pair<char, char>> abas;
    std::set<std::pair<char, char>> babs;

    for (size_t index = 0; index <= (str.size() - 3); ++index)
    {
        if ((str[index] == str[index + 2])
            && (str[index] != str[index + 1]))
        {
            if (bracket_count == 0)
                abas.insert(std::make_pair(str[index], str[index + 1]));
            else
                babs.insert(std::make_pair(str[index], str[index + 1]));
        }
    ...
    {
        if (babs.count(std::make_pair(aba.second, aba.first)) != 0)
        {
            // Got a matching bab for this aba
            return true;
        }
    }
