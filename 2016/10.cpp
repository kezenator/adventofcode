...
const std::string input = R"(value 23 goes to bot 208
...
public:
    const std::vector<int> &chips() const { return m_chips; }

    void give(int chip)
    {
        assert(m_chips.size() < 2);

        m_chips.push_back(chip);
    }

    bool has_two() const { return m_chips.size() == 2; }

    int lower() const
    {
        assert(has_two());

        if (m_chips[0] < m_chips[1])
            return m_chips[0];
        else
            return m_chips[1];
    }
...
int main(int argc, char *argv[])
{
    ...
    // Parse in the data, creating a map of rules for each bot,
    // and creating the initial bots with their initial tiles
    ...
    for (const std::string &line : lines)
    {
        if (line.substr(0, 6) == "value ")
        {
            ...
            int value = atoi(line.substr(6, space_pos - 6).c_str());

            size_t b_pos = line.find('b');
            int bot = atoi(line.substr(b_pos + 4).c_str());

            //std::cout << "value " << value << " goes to bot " << bot << std::endl;
        ...
        else if (line.substr(0, 4) == "bot ")
        {
            ...
            int bot = atoi(line.substr(pos, space_pos - pos).c_str());
            pos = space_pos;

            rule.lower_to_bot = line[pos + 14] == 'b';
            if (rule.lower_to_bot)
                pos += 14 + 4;
            else
                pos += 14 + 7;

            space_pos = line.find(' ', pos);
            rule.lower_dest = atoi(line.substr(pos, space_pos - pos).c_str());
            pos = space_pos;

            rule.higher_to_bot = line[pos + 13] == 'b';
            if (rule.higher_to_bot)
                pos += 13 + 4;
            else
                pos += 13 + 7;

            rule.higher_dest = atoi(line.substr(pos).c_str());

            //std::cout << "bot " << bot << " gives low to "
            //    << (rule.lower_to_bot ? "bot" : "output")
            //    << " " << rule.lower_dest
            //    << " and high to "
            //    << (rule.higher_to_bot ? "bot" : "output")
            //    << " " << rule.higher_dest
            //    << std::endl;

            assert(rules.count(bot) == 0);

            rules[bot] = rule;
        }
    }

    // Now, keep running a single rule each time
    // while we are able to, searching for the correct
    // comparison
    ...
    while (true)
    {
        ...
        for (const auto &entry : rules)
        {
            ...
            auto bot_it = new_bots.find(bot);
            if ((bot_it != new_bots.end())
                && bot_it->second.has_two())
            {
                const Bot &src_bot = bot_it->second;

                // Check that if we've giving it to a bot
                // that we're able to (i.e. it doesn't already have two)

                bool able = true;

                if (rule.lower_to_bot)
                {
                    auto lower_it = new_bots.find(rule.lower_dest);
                    if ((lower_it != new_bots.end())
                        && lower_it->second.has_two())
                    {
                        able = false;
                    }
                }
                ...
                if (rule.higher_to_bot)
                {
                    auto higher_it = new_bots.find(rule.higher_dest);
                    if ((higher_it != new_bots.end())
                        && higher_it->second.has_two())
                    {
                        able = false;
                    }
                }

                // If we're able, then perform the rule
                ...
                if (able)
                {
                    ...
                    if (rule.lower_to_bot)
                        new_bots[rule.lower_dest].give(src_bot.lower());
                    else
                        outputs[rule.lower_dest].push_back(src_bot.lower());

                    if (rule.higher_to_bot)
                        new_bots[rule.higher_dest].give(src_bot.higher());
                    else
                        outputs[rule.higher_dest].push_back(src_bot.higher());
    ...
    assert(outputs.count(2) == 1);

    assert(outputs[0].size() == 1);
    assert(outputs[1].size() == 1);
    assert(outputs[2].size() == 1);

    int answer2 = outputs[0][0] * outputs[1][0] * outputs[2][0];
