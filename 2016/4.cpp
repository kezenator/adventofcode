...
const std::string input = R"(fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
bool decode(const std::string &str, int &sector, std::string &decrypted)
{
    // Split data and checksum

    const size_t openpos = str.find('[');
    const std::string main = str.substr(0, openpos);
    const std::string checksum = str.substr(openpos + 1, 5);

    // Split into parts by dashes, and take last one as the sector code

    std::vector<std::string> parts;
    boost::split(parts, main, boost::is_any_of("-"), boost::token_compress_on);

    const std::string sector_str = parts.back();
    parts.pop_back();
    ...
    // Re-sort by frequency then alphabet order
    ...
    std::set<std::pair<char, int>, order_less> tosort;
    for (const auto &entry : counts)
    {
        tosort.insert(std::make_pair(entry.first, entry.second));
    }
    ...
    for (const auto &entry : tosort)
    {
        calc_checksum.push_back(entry.first);
        if (calc_checksum.size() == 5)
            break;
    }

    bool valid = (calc_checksum == checksum);

    // if valid, decrypt to text

    if (valid)
    {
        decrypted = rotate(boost::join(parts, "-"), sector);
    }
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Answer #1 : " << sector_sum << std::endl;
    std::cout << "Answer #1 : " << answer_2 << std::endl;
